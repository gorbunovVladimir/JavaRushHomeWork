...
public class Solution {
    ...
    public static boolean checkTelNumber(String telNumber) {
            ...
            else if(telNumber.matches("^(\\+{1})(\\d*\\(\\d{3}\\))?(\\d)*(-)?(\\d)*(\\d+-)?(\\d)+") && telNumber.matches("(\\D*\\d){12}") && telNumber.matches("[^\\(\\)]*(\\(\\d{3}\\))?[^\\(\\)]*")) return true;
            else if(telNumber.matches("(\\(\\d{3}\\))?(\\d+\\(\\d{3}\\))?(\\d)*(-)?(\\d)*(\\d+-)?(\\d)+") && telNumber.matches("(\\D*\\d){10}")&& telNumber.matches("[^\\(\\)]*(\\(\\d{3}\\))?[^\\(\\)]*")) return true;
            else return false;

    }
    //&& telNumber.matches("[^\\(\\)]*\\(\\d{3}\\)?[^\\(\\)]*"))
    //&& telNumber.matches("(\\d*\\+*\\-*)*\\(\\d{3}\\)?(\\d*\\+*\\-*)*"))
    //&& telNumber.matches("\\[0-9,\\+]*\\(\\d{3}\\)?\\[0-9,\\+,\\-]*"))
    //telNumber.matches("\\S*\\(\\d{3}\\)?\\S*")
