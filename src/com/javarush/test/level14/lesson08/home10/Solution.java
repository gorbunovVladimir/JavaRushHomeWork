...
public class Solution
{
    ...
    public static class NotIncapsulatedClass
    {
     ...
     private void processCastedObjects(Number numb){
             if (numb instanceof Double)
             {
                 Double a = (Double) numb;System.out.println("Is double value infinite? " + a.isInfinite());

             } else if (numb instanceof Float)
             {
                 Float a = (Float) numb;System.out.println("Is float value defined? " + a.isNaN());

             }
 }
        public NotIncapsulatedClass()
        {   List<Number> list = initList();
            printListValues(list);
            for (int i = 0; i < list.size(); i++)
            {processCastedObjects(list.get(i));}
