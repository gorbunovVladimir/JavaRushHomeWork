    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                int result=0;
                int nameDiff = ((String)stock1.get("name")).compareTo((String)stock2.get("name"));
                int dateDiff = ((Date)stock1.get("date")).compareTo((Date)stock2.get("date"));
                int cashDiff;

                double c1 =stock1.containsKey("open")?(double)stock1.get("last")-(double)stock1.get("open"):(double)stock1.get("change");
                double c2 =stock2.containsKey("open")?(double)stock2.get("last")-(double)stock2.get("open"):(double)stock2.get("change");
                cashDiff = Double.compare(c2,c1);
                //cashDiff=(int)(100*c2-100*c1);
                if (nameDiff!=0)return nameDiff;
                else if (dateDiff!=0)return dateDiff;
                else if (cashDiff!=0)return cashDiff;
                else return result;
            }
        });
    }
