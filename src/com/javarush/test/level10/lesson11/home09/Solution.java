...
public class Solution
{
    ...
    public static Map<String, Integer> countWords(ArrayList<String> list)
    {
       ...
       for (int i = 0; i <list.size(); i++)
        {list.set(i, list.get(i)); /*.toLowerCase())*/result.put(list.get(i),0);}
        ...
        {for (int k = 0; k <list.size(); k++)
          {if (list.get(i).equals(list.get(k))){
            {for( Map.Entry<String, Integer> pair : result.entrySet() )
              if( list.get(i).equals(pair.getKey()) ){ pair.setValue(pair.getValue()+1);}}}
          }
        }
        for( Map.Entry<String, Integer> pair : result.entrySet() )
            { pair.setValue((int) sqrt(pair.getValue()));}
