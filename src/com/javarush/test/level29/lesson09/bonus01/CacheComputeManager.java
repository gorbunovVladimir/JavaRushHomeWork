...
public class CacheComputeManager<Argument, Value> implements Computable<Argument, Value> {
    ...
    public Value compute(final Argument arg) throws InterruptedException {
        ...
        //Исключение, указывающее, что результат производящей значение задачи, такой как a FutureTask, не может быть получен, потому что задача была отменена.
        /*CancellationException — если ты прервал вычисление при помощи `cancel()` но пытаешся получить результат.
          InterruptedException — если тред вызвавший `get()` был прерван при помощи `interrupt()`
        */
            cache.remove(arg, f);
