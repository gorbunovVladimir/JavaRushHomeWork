...
/*меня сильно удивило, что компилятор не ругается, когда для final поля batch вызывается метод clear()
  Модификатор final указывает лишь на то, что нельзя изменить ссылку на объект, а не сам объект*/
                @Override
                public void run() {
                    while (true) {
                        try {
